...
Codex 用户级 skill 安装到 `$CODEX_HOME/skills`，默认 `~/.codex/skills`。专家 agent 说明会镜像到主 `seo` skill 的 `references/agents/`，供 Codex 读取。
...
Claude Code 使用 Claude 的 skill/agent 目录：
...
Cursor、Cline、Aider 等框架当前没有在本项目中假设统一的全局 skill 目录。推荐让它们读取本仓库的 `AGENTS.md`、`skills/`、`agents/`，并通过 `seo-agents` CLI 执行确定性检查。

Open Agent Skills 风格目录可使用：
...
完整安装说明见 `docs/INSTALLATION.md`。CLI 名称为 `seo-agents`。
